...
class FinancialSourses {
    ...
    async selectElementsToList() {        
        await this.page.getByText('Источники финансирования').first().click();
        await this.page.getByLabel('plus').locator('path').nth(1).click();
        await this.page.locator('div').filter({ hasText: /^11-Буҷети ҷумҳуриявӣ$/ }).locator('div svg').click();
        await this.page.getByText('111-Буҷети ҷумҳуриявӣ').click();
